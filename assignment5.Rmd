...
I would drop and remove the pc6 because they account for a small propotion of the variance.
...
```
yes, I have lost important infomation about mean_correct If I had dropped some components. Therefore, The dropped components are important and can not be removed.
...
```
Pc1 shows the relationship between the mean hit, mean attempt and problems attempted have high loading scores. Therefore, pc1 represent the effort of one student.<br>
PC2 might represent how students perform in the previous session will affect the performance in current sesion.<br>
PC3 might represent students' confidence in their own abilities, which means that the more correctness they get in previous session, the more confident they are.<br>
PC4 might represent knowledge that student have before the session.<br>
PC5 might represent the student's performance in dealing with a problem <br>
...
pca_1 <- prcomp(D5, scale. = TRUE)
pca_1$sdev
summary(pca_1)
plot(pca_1, type = "lines")
pca_1$rotation
loadings_1 <- abs(pca_1$rotation)
biplot(pca_1)
```
The graph suggest that programs are related to each other, since highly correlated variables point in similar directions and uncorrelated variables are nearly perpendicular to each other.
