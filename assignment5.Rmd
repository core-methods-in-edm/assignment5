...
D3=data.frame(D3,mean_correct=D1$mean_correct)
...
ggcorr(D3, method = c("everything", "pearson"),label=T,label_round = 3)
```
...
```{r}
B1=read.csv('tc-program-combos.csv')
a=c(16,69)
B2=B1[-a,]
na=B2$program
B3=B2[,na]
rownames(B3)=B2$program
pca1=prcomp(B3,scale. = T)
summary(pca1)
plot(pca1, type = "lines")
B4=data.frame(pca1$x[,1:4])
for (i in 1:67){
  for (j in 1:4){
    if (abs(B4[i,j])<1){B4[i,j]=NA}
  }
}
```

Related programs have similar distributions(same sign and similar numbers) in the same principle. If the distributions of two programs are similar, it means that they can substitute each other, showing that they are similar programs.
