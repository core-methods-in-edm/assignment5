...
PC6, as its variance is the lowest.
...
ggpairs(T1, 2:50, progress = FALSE)
ggcorr(T1[,-1], method = c("everything", "pearson"))
T2 <- select(T1, -program)
pcaT <- prcomp(T2, scale. = TRUE)
pcaT$sdev
pcaT$sdev^2
summary(pcaT)
plot(pcaT, type = "lines")
pcaT$rotation
loadings <- abs(pcaT$rotation)
biplot(pcaT)

#As there are arrows of different programs towards similar directions.
```
