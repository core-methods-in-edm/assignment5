...
I would drop PC6 and PC7, because PC1, PC2, PC3, PC4, and PC5 can represent about 89% and the Proportion of Variance of PC6 and PC7 are small.
## Part II
...
#I cannot see the relationship between mean_correct and mean_attempt as well as the relationship between mean_correct and mean_confidence. But this maybe not very important.
```
...
D11 <- read.csv("tc-program-combos.csv", header=TRUE)
library(ggplot2)
library(GGally)
ggpairs(D11, 2:68, progress = FALSE)
ggcorr(D11[,-1], method = c("everything", "pearson"))
