...
#The outcome shows that "mean_correct" is strongly related to the outcome "prior percent correct".
```
...
```
We might lose important information if we dropped some components, which depends on which components do we drop. If we drop PC3 and PC5, the information may not be lost as the two components are not related mean_correaltion. Otherwise, the rest components have either positive or negative relationships with it, the drop of them will cause information loss.
...
#PC1: "mean_hint" and "mean_attempt" has higher loading scores, which means students' efforts to try to get things right.
#PC2: "prior_prob_count" and "prior_percent_correct" has higher loading scores, which means students' accumulated efforts and level of difficulties for individual students.
#PC3: "mean_confidence" has higher loading scores, which means students' recognition to their confidence level
...
ggcorr(tc_program_df [,-1], method = c("everything", "pearson"))
tc_program_pca <- prcomp(tc_program_df[,-1], scale. = TRUE)

summary(tc_program_pca)
plot(tc_program_pca, type = "lines")

TCloadings <- abs(tc_program_pca$rotation)

biplot(tc_program_pca)
```

I think for each PC, programs have high loading socres share some commonalities, these commonalities make students feel they are related. For example, programs with high scores in PC1 include Change.Leadership, Economics.and.Education, Education.Policy, Arts.Administration, Politics, School.Principals, Social.Organizational.Psychology, Private.School.Leadership, Cooperation.and.Conflict.Resolution and Leadership. They are all around management, leadership and policy. PC2 programs are around psychology, PC3 programs are more quantatitive and etc.. 
