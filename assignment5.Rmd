...
I would like to remove the PC5 and PC6 since they only account for a small proportion of the variance.
...
## Yes, I would lost important information if I drop both PC5 and PC6 since the correlation between PC6 and mean correct is -0.393, which is not small comparing to others.
...
```{r}
pcaDF <- prcomp(DF2, scale = TRUE)
pcaDF$sdev
pcaDF$sdev^2
summary(pcaDF)
plot(pcaDF, type = "lines")
pcaDF$rotation
loadingsDF <- abs(pcaDF$rotation)
biplot(pcaDF)
```
