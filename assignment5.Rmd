...
*I will drop the last one, because the first five reach more than 90 percents for the first time*
...
*I think we don't lose much information if we drop PC3 and PC5 because they have littel correlation with mean_correct*
...
P <- ifelse(abs(pca2$rotation)>0.2,pca2$rotation,0)
P[,1:33]
biplot(pca2)
```

*Psychology programs are in the same direction, and education programs are in same direction. The related prgrams will have similar components in PCA variables*
