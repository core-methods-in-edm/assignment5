...
plot(pca, type = "lines", main = "PCA Line")
```

## Decide which components you would drop and remove them from your data set.

## ANSWER；
This data seems to be more discernible than the chart. From the above results,the The variance changes from PC3 to PC6 are not very obvious.By using PCA, If the first few principle components account for a large proportion of the variance in the data, PCA was successful. So I may will remove PC4, PC5 and PC6.
...
## ANSWER：
Based on the new result, we saw that PC1,PC2 and PC3 have the strong positive relationship with mean_correct. So we should keep these three components. PC5 and PC6 seems like is the two components which have negative relationship with mean_correct. Discarding these two components may result in the loss of some information to analysis.Therefore, conservative speaking, PC5 maybe is the only components we can drop.
...
## ANSWER: PC1 maybe represent how students familiar with the question or do they have any backgrond information.PC2 maybe represent the frequency of answering the question 
...
TC<-read.csv("tc-program-combos.csv")
ggcorr(TC[,-1], method = c("everything", "pearson"))
TC1<-TC[,-1]
pcaTC <- prcomp(TC1, scale = TRUE)
pcaTC$sdev
pcaTC$sdev^2
summary(pcaTC)
plot(pcaTC, las=0.1, type = "line", main = "TC students PCA")
pcaTC$rotation
loadingsTC <- abs(pcaTC$rotation)
biplot(pcaTC)

# how to change the lable size in biplot?  I try so many ways but cannot run successfully. 
```
