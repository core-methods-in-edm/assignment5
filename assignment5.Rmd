...
## I would move PC 7, since PC 7 has the least influence on clustering the dataset. 
## Part II
...
```{r}
DF <- read.csv("tc-program-combos.csv")

DF1 <- DF[,-1]

mypca <- prcomp(DF1, scale = TRUE)
...
DF2 <- data.frame(DF, mypca$x[,c(1:2)])

ggplot(DF2, aes(PC1,PC2, col = program, fill = program)) + 
