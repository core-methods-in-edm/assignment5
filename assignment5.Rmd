...
#Yes, dropping PC6 can lost important information about mean_correct.
...
#Although princomp does not generate the eigenvalues directly for us, we can print a list of the standard deviation of the variance accounted for by each component
pca2$sdev
#Convert this into variance accounted for we can square it, these numbers are proportional to the eigenvalue
pca2$sdev^2
#summary(pca2) give the proportion of variance accounted for by each component
summary(pca2)
#Find out which components to keep and which to drop
plot(pca2, type = "lines")
#Print out the loadings for the components you generated
pca2$rotation
#The eigenvectors are a little difficult to interpret. It is much easier to make sense of them if we make them proportional within each component
loadings2 <- abs(pca$rotation) #abs()make all eigenvectors positive
#Now examine your components and try to come up with substantive descriptions of what some might represent?
biplot(pca2)

#The arrow of similar fields go to the same direction, "go together".
```
