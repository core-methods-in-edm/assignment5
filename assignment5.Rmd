...
I think we can remove PC5 and PC6 as they account for less proportion of the variance
...
D3 <- subset(D2, select=-6)
D3 <- subset(D3, select=-6)
...
library(ggplot2)
library(GGally)
ggpairs(D3,2:6,progess=FALSE)
ggcorr(D3[,-1], method = c("everything", "pearson"))

#The dropped components seem not too important and can be removed.
...
```{r}
#Looking at the plot, we can see that most clinic and physical programs go at the same direction, and education-related programs go at the opposite direction, each gathering together. The relationships are like this because these programs share some of the same content knowledge.
```
