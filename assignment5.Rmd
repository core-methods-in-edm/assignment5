...
## Removing pc6, as it accounts for the lowest proportion of variance
...
## pc1: mean_hint, mean_attempt (measures student engagement)
## pc2: prior_percent_correct, prior_prob_count (measures student knowledge)
## pc3: mean_confidence (measures student confidence level)
## pc4: prior_prob_count (measures student knowledge)
## pc5: mean_attempt, problems_attempted (measures student resilience level)
## pc6: n/a
...
pca2$sdev^2 #To convert this into variance accounted for we can square it, these numbers are proportional to the eigenvalue

summary (pca2) #A summary of our pca will give us the proportion of variance accounted for by each component

plot (pca2, type = "lines") #We can look at this to get an idea of which components we should keep and which we should drop

pca2$rotation #Examine the eigenvectors, notice that they are a little difficult to interpret. It is much easier to make sense of them if we make them proportional within each component

loadings2 <- abs (pca2$rotation) #abs() will make all eigenvectors positive

biplot (pca2)

##biplot provides a visualization of which variables "go together," and variables that possibly represent the same underlying construct point in the same direction; this suggests that there are relationships between these programs
