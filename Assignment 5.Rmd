...
#The most important two behaviors are hints and prior_prob_count because hints is the first variable used and prior_prob_count is the second variable used to determine the result.
...
prediction = predict(score_ctree, D2)
D2$advice <- ifelse(D2$score <=0.4, "intervene", ifelse(D2$score > 0.4 & D2$score <=0.8, "monitor", "no action"))
matrix <- table(D2$advice, prediction)
accuracy = matrix[3]/sum(matrix)
1-accuracy
```

#The error rate is 0.42 on the predictions of the new data.
