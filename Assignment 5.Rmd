...
#kids who request many  hints and kids with lower  prior % correct
...
#Not accurate...only 58% correct. Model is having kids go through interventions they might not need


```{r}
D2$advice <- ifelse(D2$score <=0.4, "intervene", ifelse(D2$score > 0.4 & D2$score <=0.8, "monitor", "no action"))
D2$accuracy <- ifelse(D2$prediction == D2$advice, 1, 0)
hist(D2$accuracy)
sum(D2$accuracy)/length(D2$accuracy)
```
