...
#Plot tree
```{r}
tiff("tree.tiff", units="in", width=18, height=10, res=300)
plot(score_ctree)
dev.off()
```
...
A: if # of hints is too high/ if prior_percent_correct is too low.
...
```{r}
D3 <- select(D2,prediction)
t <- count(D3,prediction)

# Pie Chart
bp<- ggplot(t, aes(x="", y=n, fill=prediction))+
